...
TXT: dict[str, tuple[str, str]] = {
    ...
    "sc_next_on": ("Wechsel in Staatsanleihen, wenn der Tages-VaR über {hi} steigt (aktuell {v}) "
                   "oder wenn {all}.",
                   "Switch to Treasuries if daily VaR rises above {hi} (now {v}), or if {all}."),
    "sc_end_calm": ("der Tages-VaR über {lo} steigt", "daily VaR rises above {lo}"),
    "sc_end_trend": ("der 30-Tage-Schnitt unter den 200-Tage-Schnitt fällt (aktuell {g} darüber)",
                     "the 30-day average falls below the 200-day average (now {g} above)"),
    "sc_end_dip": ("der Kurs sich auf weniger als {lim} unter dem Hoch erholt (aktuell {f} darunter)",
                   "the price recovers to less than {lim} below its high (now {f} below)"),
    "sc_and_all": (" und ", " and "),
